...
import { Grid, Input, Button, InputAdornment, IconButton } from '@material-ui/core';
    ...
    return(
      <Fragment>
          ...
          >
            ...
            <TableContainer
                  component={Paper}
                  sx={{ maxHeight: window.innerHeight * 0.7 }}
            >
              <Table
                sx={{ minWidth: 650 }}
                size="small"
              ...
              >
                  ...
                  <TableBody>
                      {records.slice(startRecordIndex, endRecordIndex).map((r, key)=>{
                          ...
                          return (
                            <TableRow style={lineStyle} key={key}>
                                {collumns.map((c, key) => 
                                    <TableCell 
                                      style={selectCell.id === r[tableStruct.idField] && selectCell.field === c.field ? styles.selectTableTd : {}}
                                      onClick={() => setSelectCell({id: r[tableStruct.idField], field: c.field})}
                                      key={key}
                                    >
